...
"""
    ...
    if not allowed:
        reason = guardrail_reason or (
            "LangTrip AI can only help with travel-planning requests. "
...
def flight_agent(state: TravelState):
    ...
    # Try AviationStack MCP first
    try:
        airports = asyncio.run(aviation_mcp_call("list_airports"))
        airlines = asyncio.run(aviation_mcp_call("list_airlines"))
        airports_str = str(airports)[:3000]
        airlines_str = str(airlines)[:3000]
    except Exception as exc:
        print(f"AviationStack MCP unavailable, using flight tool fallback: {exc}")
        try:
            from tools.flight_tool import search_flights
            direct_result = search_flights(query)
            if direct_result and "error" not in direct_result.lower():
                airports_str = direct_result[:3000]
        except Exception:
            pass

    prompt = FLIGHT_AGENT_PROMPT.format(
        query=query,
        airport_data=airports_str or "Standard international airport registry and major airline routes",
        airline_data=airlines_str or "Major international flag carriers and low-cost airlines",
    )

    try:
        response = llm.invoke(
            [
                SystemMessage(content="You are an expert aviation and flight route planning specialist. Provide clear, realistic, and practical airline advice, likely airports, durations, and booking tips."),
        ...
        flight_data = (
            "### ✈️ Flight Intelligence & Route Overview\n\n"
            f"- **Route**: Direct and 1-stop options for {query}\n"
            "- **Recommended Airlines**: Air India, Emirates, Singapore Airlines, ANA, Qatar Airways, IndiGo\n"
            "- **Flight Duration**: 5 to 9 hours depending on direct or transit connections\n"
            "- **Booking Tip**: Compare fares across aggregators and book 4-6 weeks in advance for best seasonal rates."
        )
...
def _format_hotel_output(raw_results, query: str) -> str:
    prompt = f"""
Format these hotel search results into a clean, beautiful Markdown hotel guide for a traveler:
User query: {query}
Hotel raw data: {str(raw_results)[:3000]}

Generate:
- 3 to 4 recommended stays (Luxury, Mid-Range, Budget)
- Best neighborhoods to stay in
- Approximate price range per night & key amenities
"""
    try:
        res = llm.invoke([
            SystemMessage(content="You are an expert accommodation and hotel scout."),
            HumanMessage(content=prompt)
        ])
        return res.content
    except Exception:
        return (
            "### 🏨 Recommended Stays & Neighborhood Guide\n\n"
            "- **Luxury & Boutique**: 5-star central stays with premium amenities ($220–$380/night)\n"
            "- **Comfort Mid-Range**: Modern hotels near transit hubs with breakfast included ($90–$160/night)\n"
...
def hotel_agent(state: TravelState):
    ...
    try:
        raw_hotel = asyncio.run(tavily_mcp_search(query))
    except Exception as exc:
        print(f"HOTEL AGENT MCP ERROR: {type(exc).__name__}: {exc}", flush=True)

    formatted_hotel = _format_hotel_output(raw_hotel or state["user_query"], state["user_query"])

    return {
        "hotel_results": formatted_hotel,
        "messages": [AIMessage(content="Hotel information processed.")],
        "llm_calls": state.get("llm_calls", 0) + 1,
    }


def _format_weather_output(weather_data, forecast_data, city: str) -> str:
    prompt = f"""
Format this weather and forecast information into a clean, beautiful Markdown travel forecast:
Destination / City: {city}
Current Weather raw data: {str(weather_data)[:2000]}
Forecast raw data: {str(forecast_data)[:2000]}

Generate:
1. Current Climate & Temperature Range
2. 5-Day Travel Forecast Summary
3. Packing & Gear Advice (layers, footwear, rain gear)
"""
    try:
        res = llm.invoke([
            SystemMessage(content="You are an expert travel meteorologist and packing advisor."),
            HumanMessage(content=prompt)
        ])
        return res.content
    except Exception:
        return (
            f"### 🌦️ Climate & Weather Forecast for {city}\n\n"
...
def weather_agent(state: TravelState):
    ...
    # Clean city name
    clean_city = city.split("&")[0].split(",")[0].split(" and ")[0].strip() if city else "Destination"

    weather_data = None
    forecast_data = None

    try:
        weather_data = asyncio.run(weather_mcp_search(clean_city))
        forecast_data = asyncio.run(forecast_mcp_search(clean_city))
    except Exception as exc:
        print(f"WEATHER AGENT MCP ERROR: {type(exc).__name__}: {exc}", flush=True)

    formatted_weather = _format_weather_output(weather_data, forecast_data, clean_city)
